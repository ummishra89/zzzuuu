 ...
 */
class GetMenteeRequester(private var mentorId: Long?, pageNo: Int, private var listener: UserDetailsResponseListener?) : BaseRequester() {
    private var pageNo: Int? = pageNo

    fun execute() {
        doAsync {
            //run in background
            val mentorzApiResponse: MentorzApiResponse? = HttpController.getMenteeList(mentorId!!, pageNo!!)
    ...
    private fun callBack(mentorzApiResponse: MentorzApiResponse?) {
        ...
        if (mentorzApiResponse?.statusCode == HttpURLConnection.HTTP_OK) {
            ...
            if (mentorResponse != null) {
                listener?.onMenteeSuccess(mentorResponse.userList!!)
            }
            return
