 ...
 */
class GetFollowersRequester(mentorId: Long, pageNo: Int, private var listener: UserDetailsResponseListener?) : BaseRequester() {
    ...
    fun execute() {
        doAsync {
            //run in background
            val mentorzApiResponse: MentorzApiResponse? = HttpController.getFollowers(mentorId!!, pageNo!!)
    ...
    private fun callBack(mentorzApiResponse: MentorzApiResponse?) {
        ...
        if (mentorzApiResponse?.statusCode == HttpURLConnection.HTTP_OK) {
            ...
            if (mentorResponse != null) {
                listener?.onFollowersSuccess(mentorResponse.userList!!)
            }
            return
